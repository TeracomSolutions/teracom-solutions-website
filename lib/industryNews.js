...
export function parseRssItems(
  xml,
  { allowedHost = 'sen.news', limit = 12, excludeCategories = [], excludeTitle = null, stripLeadDash = true } = {}
) {
  const items = [];
  ...
  while ((match = itemRegex.exec(xml)) !== null && items.length < limit) {
    ...
    const title = decodeHtmlEntities(readTag(itemXml, 'title')).trim();
    ...
    items.push({
      ...
      excerpt: toExcerpt(readTag(itemXml, 'description'), { stripLeadDash }),
    });
  }
...
export const NEWS_SOURCES = [
  ...
  { id: 'electrical', label: 'Electrical', source: 'Electrical Connection', site: 'https://electricalconnection.com.au', feed: 'https://electricalconnection.com.au/feed/', host: 'electricalconnection.com.au', excludeCategories: ['Transport', 'Contributors'] },
  ...
  { id: 'smart-home', label: 'Smart home', source: 'The Ambient', site: 'https://www.the-ambient.com', feed: 'https://www.the-ambient.com/feed/', host: 'the-ambient.com' },
];
...
export async function getSourceNews(source, limit = 12) {
  try {
    ...
    const items = parseRssItems(await response.text(), {
      allowedHost: source.host,
      limit,
      excludeCategories: source.excludeCategories || [],
      excludeTitle: source.excludeTitle || null,
      stripLeadDash: Boolean(source.stripLeadDash),
    });
    return items.map((item) => ({ ...item, sourceId: source.id, source: source.source, industry: source.label }));
  } catch {
    return [];
  }
}
