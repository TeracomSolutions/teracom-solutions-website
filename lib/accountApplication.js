    ...
    },
    {
      ...
      declarations: [
        ...
        },
        {
          ...
          required: true,
          termsLink: true,
          text: `I have read and accept the Teracom Solutions terms and conditions, version ${TERMS_VERSION}, effective ${TERMS_EFFECTIVE}.`,
        },
        {
          id: 'guarantee',
          required: true,
...
export function formatApplication(values, directors = []) {
  ...
  // What was actually agreed to, in a form that survives the document being
  // superseded. The version alone is not enough: a version number can be
  // reused by mistake, and the hash cannot. Whoever reads this record in two
  // years can fetch that exact document and prove it is the one.
  if (values.terms) {
    lines.push(
      '',
      'Terms accepted:',
      `- Version ${TERMS_VERSION}, effective ${TERMS_EFFECTIVE}`,
      `- Document SHA-256 ${TERMS_CONTENT_SHA256}`
    );
  }
