  ...
  steps: [
    ...
    },
    {
      ...
      fields: [
        ...
        { id: 'abn', label: 'ABN', type: 'text', required: true, help: 'We check this against ABN Lookup.' },
...
export function missingFrom(step, values, directors = []) {
  const missing = visibleFields(step, values)
    .filter((field) => field.type !== 'heading' && field.required && !String(values[field.id] || '').trim())
