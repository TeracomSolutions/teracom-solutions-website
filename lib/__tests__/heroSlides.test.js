...
test("a news slide uses the story's own picture when the feed has one", () => {
  const slides = buildHeroSlides({
    security: [{ ...story('A security story'), imageUrl: 'https://sen.news/photo.jpg' }],
  });
  assert.equal(slides[2].image, 'https://sen.news/photo.jpg');
  assert.equal(slides[2].imageExternal, true);
  // Empty alt: the headline sits right beside it and says the same thing.
  assert.equal(slides[2].imageAlt, '');
});

test('a news slide falls back to Teracom artwork when the feed has no image', () => {
  const slides = buildHeroSlides({ security: [story('A security story')] });
  assert.match(slides[2].image, /^\/assets\//);
  assert.ok(!slides[2].imageExternal);
  assert.ok(slides[2].imageAlt, 'Teracom artwork still needs alt text');
});
