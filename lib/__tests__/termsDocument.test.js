...
test('the document is complete: Part A, five schedules and the annexure', () => {
  assert.deepEqual(
    termsParts.map((part) => part.id),
    ['part-a', 'schedule-1', 'schedule-2', 'schedule-3', 'schedule-4', 'schedule-5', 'annexure-a']
  );
...
test('the version and effective date are consistent throughout', () => {
  assert.equal(TERMS_VERSION, '3.0');
  assert.equal(TERMS_EFFECTIVE, '1 October 2026');
  assert.match(TERMS_PDF, /Teracom-Terms-and-Conditions-v3\.0\.pdf$/);
  ...
  assert.ok(termsVersionHistory && termsVersionHistory.includes('v3.0'));
});
