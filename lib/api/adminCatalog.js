...
// Soft delete by default: the product disappears from the store but keeps
// its history. permanent removes the row for good (backend requires it to
// be inactive first).
export async function deactivateProduct(token, productId, { permanent = false } = {}) {
  return backendFetch(`/staff/store-catalog/products/${encodeURIComponent(productId)}`, {
    method: 'DELETE',
    token,
    searchParams: { permanent: permanent ? 'true' : undefined },
  });
}
