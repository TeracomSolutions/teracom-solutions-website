...
export default function CategoryPage({ params }) {
  ...
  return <main id="main-content"><section className="hero hero-product hero-shallow"><div className="container hero-layout"><div className="hero-copy"><Breadcrumbs items={[{ name: 'Teracom Store', href: '/store' }]} current={category.title} /><h1>{category.title}</h1><p className="lead">{category.description}</p></div></div></section><section className="section section-spacious"><div className="container">{products.length > 0 ? <CategoryProductGrid products={products} isSignedIn={Boolean(token)} /> : <div className="form-note-banner" role="status">Full product listings for {category.title} are coming soon. In the meantime, <Link href="/#contact" style={{color:'var(--text)',textDecoration:'underline'}}>contact us</Link> for current stock and pricing.</div>}</div></section></main>;
}
