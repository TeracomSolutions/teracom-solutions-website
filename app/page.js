...
// These 3 logo files have no real transparency (an opaque JPEG/GIF, or a
// PNG whose alpha channel is fully opaque) -- the white-silhouette
// .logo-mono filter (brightness(0) then invert(1)) needs transparency to
// work, and without it just renders the whole rectangular image as a
// solid white block. Confirmed by checking the live rendered page after
// the filter shipped, not guessed up front. These get a small white
// backing card behind the original logo instead; everything else stays
// a plain white silhouette directly on the dark background.
const OPAQUE_LOGO_FILES = ['inner-range.png', 'kantech.jpg', 'dsc.gif'];
